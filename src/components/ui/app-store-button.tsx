...
export function AppStoreButton({
  className,
  href,
  // El sitio habla cuatro idiomas y los rótulos ya viven en los diccionarios;
  // el original los traía fijos en inglés. Se dejan como valores por defecto.
  pre = "Download on the",
  name = "App Store",
  ...props
  ...
  const contenido = (
    <>
      ...
      <div className="text-left flex flex-col items-start justify-center pr-2">
        <span className="text-[10px] leading-none tracking-tighter">{pre}</span>
  ...
  return (
    // Cuando hay ficha, el `<a>` ES el botón: `asChild` traslada las clases al
    // ancla. Envolver el `<button>` en un `<a>` sería HTML inválido —dos
    // elementos interactivos anidados— y daría dos paradas de tabulación.
    <Button asChild={Boolean(href)} className={cn("h-11 gap-2", className)} {...props}>
      {href ? (
