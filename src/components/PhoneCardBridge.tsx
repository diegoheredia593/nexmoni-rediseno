...
export function PhoneCardBridge({
  label,
  phrase,
}: {
  label: string;
  ...
  const cardOpacity = useTransform(progress, [0, 0.4, 1], [0.2, 0.48, 1]);
        ...
        >
          <defs>
            ...
            <radialGradient id="bridge-card-glow">
              <stop offset="0" stopColor="var(--acento-alto)" stopOpacity=".5" />
          ...
          <path
            className="product-bridge__guide"
            d="M230 220 C350 220 365 118 455 118 C548 118 550 198 650 198"
          />
          <motion.path
            className="product-bridge__flow"
            d="M230 220 C350 220 365 118 455 118 C548 118 550 198 650 198"
            style={{ pathLength: reducedMotion ? 1 : progress }}
          />
          <motion.circle
            className="product-bridge__pulse"
            r="8"
            cx={reducedMotion ? 650 : dotX}
            cy={reducedMotion ? 198 : dotY}
            filter="url(#bridge-pulse)"
          />

          <motion.g style={{ opacity: reducedMotion ? 1 : cardOpacity }}>
            <motion.ellipse
              cx="790"
              cy="198"
              rx="196"
              ry="142"
              fill="url(#bridge-card-glow)"
              style={{ opacity: reducedMotion ? 0.7 : glowOpacity }}
            />
            <rect className="product-bridge__card" x="650" y="110" width="280" height="176" rx="18" />
            <rect className="product-bridge__chip" x="682" y="160" width="43" height="34" rx="6" />
            <path className="product-bridge__card-line" d="M682 226h150M682 250h74" />
            <text className="product-bridge__visa" x="844" y="254">VISA</text>
          </motion.g>
        </svg>
      </div>
    </section>
